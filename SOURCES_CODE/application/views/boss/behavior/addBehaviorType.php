<link rel="stylesheet" href="<?php echo base_url();?>css/tableBox.css">    
<link rel="stylesheet" href="<?php echo base_url();?>css/font.css">
        <style>
	body{
		font-family: thaisanslite_r1 Vera Serif Bold;
		margin:0;
		margin-top:-18px;
		}
</style>
<div class="table" align="center" >
<form action="<?php echo base_url();?>index.php/boss/addBehaviorMagTypeAction" method="post">
  <table width="100%" border="0" align="center" cellpadding="7" cellspacing="3">    
 <tr>   
    <th colspan="2" align="center" valign="baseline" nowrap="nowrap" ><p>เพิ่มข้อมูลหมวดหมู่พฤติกรรมทัตสุขภาพ</p></th>
   	</tr>

    <tr>    
           <td width="61" align="center" valign="middle" nowrap="nowrap" ><p>ชื่อหมวดหมู่</p></td>  
       <td width="158" align="center" valign="middle" nowrap="nowrap" ><p><input name="behaviorTypeName"  type="text"  required id="behaviorTypeName" size="50" ></p></td>

   
    </tr>


      <tr>    
           <td colspan="2" align="center" valign="middle" nowrap="nowrap" ><p>
             <input type="submit" name="save" id="save" value="บันทึก">
             &nbsp;&nbsp;
             <input type="button" name="cancle" id="cancle" onClick="parent.jQuery.fancybox.close();" value="ยกเลิก">
           </p></td>  
    </tr>
</table>
</form>

</div>

